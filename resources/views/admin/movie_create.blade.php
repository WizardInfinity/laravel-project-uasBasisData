{{-- resources/views/movie_create.blade.php --}}
@extends('layout')
                ...
                <div class=" sm:px-10 ">
                    ...
                    <form class="space-y-6" action="{{ route('admin.movie.store') }}" method="POST"
                        ...
                        <div>
                            ...
                            <div class="mt-1">
                                <textarea id="sinopsis" name="sinopsis" type="text" rows="3" autocomplete="off" required
                                    ...
                                    placeholder="Masukkan Sinopsis Movie">{{ old('sinopsis') }}</textarea>
                            </div>
                            @error('sinopsis')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <div class="mt-1">
                                <input id="tahun_rilis" name="tahun_rilis" type="number" min="1900"
                                    max="{{ date('Y') }}" autocomplete="off" required value="{{ old('tahun_rilis') }}"
                        ...
                        <div id="preview-container" class="mt-4 hidden">
                            <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Preview Poster:</p>
                            <img id="poster-preview" src="#" alt="Preview Poster"
                                class="mt-2 max-h-64 rounded-lg shadow-md border dark:border-gray-600">
                        </div>
                        <div class="flex space-x-3">
                            <button type="submit"
                                class="flex-1 flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-gray-800 transition-colors duration-200">
                                Tambahkan Movie
                            </button>
                        </div>
                    </form>
